"""Train the original Vaswani encoder-decoder on the frame-based HI -> HF dataset.

EXPERIMENT-10 COPY (grokking sweep)
-----------------------------------
This is a *verbatim copy* of
`experiments/06_vaswani_original_hi_hf_frames_heads_4_layers_4/run.py` with TWO
added knobs threaded into Seq2SeqTrainingArguments: `--weight-decay` and
`--lr-scheduler`. We copy rather than edit 06 because (a) repo convention forbids
editing existing experiments, and (b) classic grokking (Power et al.) leans
heavily on weight decay and a non-decaying LR, which 06 hardcodes to 0.0 /
'linear'. Everything else (model, tokenizer defaults,
eval metric, Hub push) is identical to 06, so 10's vaswani arch stays directly
comparable to 06/09. Driven by `experiments/10_multi_seed_grokking/run.py`.

WHAT'S DIFFERENT FROM EXPERIMENT 04
-----------------------------------
* Data source: a HuggingFace Hub dataset built by experiment 05's frame
  generator (e.g. `kylelovesllms/hi_hf_frames_d3_random_100`). Pass the repo
  id via `--dataset`. The dataset already has `train`/`validation`/`test`
  splits partitioned by *frame* (not by row), so we do NOT repartition by
  depth here -- the structural split is baked into the dataset.
* Split policy at build time = `random_frame`, so train and val/test share
  no parse skeletons but the depth distribution is roughly matched across
  splits. Val/test exact-match therefore measures generalization to unseen
  frames at the same depths, not recursion holdout (that would be the
  `held_out_depth` variant, a separate experiment).
* Architecture width restored to the paper-ish default (`d_model=128,
  d_ff=512`) -- the "original" in the experiment name. Shape is
  `n_heads=4, n_layers=4` as specified.

WANDB
-----
project=`constituent-geometry`,
run=`06_vaswani_original_hi_hf_frames_heads_4_layers_4_random_depth_3`.
...
---
Local smoke test:
    uv run python experiments/10_multi_seed_grokking/runners/vaswani_run.py \\
...
def main() -> None:
    ...
    p.add_argument(
        "--output-dir",
        default="experiments/06_vaswani_original_hi_hf_frames_heads_4_layers_4/results",
    )
    p.add_argument("--src", default="hi", help="encoder input column")
    p.add_argument("--tgt", default="hf", help="decoder target column")

    # --- model knobs --------------------------------------------------------
    # "original" Vaswani-shape: paper-ish width with the specified head/layer
    # count. Different from 04 (which used d_model=64, d_ff=256 for a small
    # capacity probe).
    ...
    # --- grokking knobs (the ONLY additions vs experiment 06) ---------------
    # AdamW L2 regularization. 06 leaves this at the HF default (0.0); grokking
    ...
    # LR schedule. 06 uses the HF default 'linear' (warmup then decay to 0),
    ...
    p.add_argument(
        ...
        help="If set, evaluate on a fixed RANDOM subset of this many val/test rows "
             "(shuffle seed 0) instead of the full split. Keeps per-step "
             "generation-eval tractable on large datasets.",
    )

    p.add_argument("--wandb-project", default="constituent-geometry")
    p.add_argument(
        "--run-name",
        default="06_vaswani_original_hi_hf_frames_heads_4_layers_4_random_depth_3",
    )
    ...
    if args.max_eval_samples is not None:
        n_val = min(args.max_eval_samples, len(val_ds))
        n_test = min(args.max_eval_samples, len(test_ds))
        val_ds = val_ds.shuffle(seed=0, keep_in_memory=True).select(range(n_val), keep_in_memory=True)
        test_ds = test_ds.shuffle(seed=0, keep_in_memory=True).select(range(n_test), keep_in_memory=True)
    print(
    ...
    # --- generalization: ENTIRE validation set (uncapped), per request -------
    # Per-step eval ran on a capped subset (--max-eval-samples) to keep training
    # tractable; this one-off pass measures the (best) model on the FULL val split.
    val_full_tok = raw["validation"].map(tokenize, batched=True, remove_columns=cols)
    val_full_metrics = trainer.evaluate(eval_dataset=val_full_tok, metric_key_prefix="val_full")
    print("val_full (ENTIRE validation set):", val_full_metrics)
