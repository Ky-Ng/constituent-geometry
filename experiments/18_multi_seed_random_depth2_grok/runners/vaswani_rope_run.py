...
`experiments/07_vaswani_RoPE_hi_hf_frames_heads_4_layers_4/run.py` with TWO added
knobs threaded into Seq2SeqTrainingArguments: `--weight-decay` and
`--lr-scheduler`. We copy rather than edit 07 because repo convention forbids
editing existing experiments, and grokking needs weight decay and a non-decaying
LR (07 hardcodes 0.0 / 'linear'). The default
`--tokenizer` still points at 07's artifact dir, which stays in the repo, so the
copy resolves it identically when run with cwd=REPO_ROOT. Driven by
`experiments/10_multi_seed_grokking/run.py`.
...
---
Local smoke test:
    uv run python experiments/10_multi_seed_grokking/runners/vaswani_rope_run.py \\
...
def main() -> None:
    ...
    # --- grokking knobs (the ONLY additions vs experiment 07) ---------------
    # AdamW L2 regularization. 07 leaves this at the HF default (0.0); grokking
    ...
    # LR schedule. 07 uses the HF default 'linear' (warmup then decay to 0),
