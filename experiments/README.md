...
| 09 | `09_multi_seed` | Ready | Multi-seed reproduction of 06/07/08 (seeds 42–51, `random_frame` depth-3, 30 epochs) to test whether their single-seed attention heatmaps are seed-robust. A thin launcher shells out to the existing 06/07/08 `run.py` (no model logic duplicated), saving 30 checkpoints locally (`--no-push`). Supports packed (3 jobs, seeds share one GPU) or array (1 GPU/run) submission. Cross-seed heatmap comparison is two-stage and CPU-only: `extract_attention.py` (30-task array, one job per arch×seed, caches attention for 12 depth-stratified prompts) then `gather_heatmaps.py` (renders cosine-reordered seeds-as-rows grids + a consistency CSV). |
